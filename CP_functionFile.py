...
def rajoutBout(a, indExtrem):
    indExtrem = np.append(1, indExtrem)
...
def selectStep(acq, event):
    ...
    indMax = np.ravel(maxLocal(data,1))
...
def plotPosi(acq, position, axe, ax, event=0):
    ...
    Min, Max = minLocal(data)+first_frame-1, maxLocal(data)+first_frame-1
    pas = np.array(selectStep(acq, position))+first_frame-1


    # Plot part
    ax = plotEvent(acq, ax)
    ax.plot(np.array(range(first_frame-1, last_frame)), data, 'k')
    ax.plot(Min, data[Min-(first_frame-1)], 'o b')
    ax.plot(Max, data[Max-(first_frame-1)], 'o', color='purple')
    ax.axvline(x = pas[0]+1, color = 'cyan')
    ax.axvline(x = pas[1]+1, color = 'cyan')
...
def mean_distance(trainingSet, eventLabel, eventContext, capteur, genre, taux):
    ...
    for acr in trainingSet:     # On calcule la distance à travers tous les training files
        ...
        # print('Position extrem : ', positionExtrem)
    # print('ArrayDistance :', arrayDist)
...
def pasR(acq, capteurSet):
    if capteurSet[0][0] == 'L':     # On regarde si le premier capteur concernent le pied gauche
        ...
        capteur = 'RANK'
    taux = 0.8      # le taux, pour fixer le seuil, afin d'éviter le min et max qui se chavauchent
    ...
    pasRank = indMax[data[indMax] > seuil]
    pasPlus = pasRank                  # On prend que ce qui dépasse du seuil

    if pasRank[0] > 5:
        pasPlus = np.append(1, pasRank)

    if pasRank[-1] < len(data) - 5:                            # On rajoute la premiere et derniere frame
...
def calcPredic(acr, moyenne, capteurSet, genre):
    ...
    for i in range(len(pasRank)-1):         # On parcourt les pas
        ...
        for capteur in capteurSet:          # Chaque capteur fait sa prédiction
            ...
            seuil = np.mean(np.max(data)*taux + np.min(data)*(1-taux))
            seuil2 = np.mean(np.max(data)*taux2 + np.min(data)*(1-taux2))
            Min, Max = minLocal(data), maxLocal(data, 1)        # Extremum, mais juste dans le pas

            if genre[capteur] == 'max':
                # print('[', capteur, '] : Maxs : ', data[Max], '- Seuil :', seuil)
            ...
            if genre[capteur] == 'min':
                ...
                # print(capteur, ' -- pas :', pasRank[i], '- Min :', Min, '- moyenne :', moyenne[capteur])
...
# In : les fichiers tests, etc... (voir mean_distance)
...
def exempleFinal(acfin, moyenne, capteurSet, genre, weight, eventLabel, eventContext):
    # acfin = testSet[np.random.randint(len(testSet))]
    ...
    # print('Prediction moyenne :', meanPrediction)
